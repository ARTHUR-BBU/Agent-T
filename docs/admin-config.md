...
|----------|------|--------------|--------------|
| **补盲候选** | **开**（`BLIND_SPOT_ENABLED=true`，亦接受 `1`/`yes`） | 对「未找到」条目和**评分卡点名的缺口**条目（M3.5 定向化：模型在评分卡 JSON 每段输出 `gap_item_ids`，系统把点名条目纳入补盲靶点），模型可提出「候选需关注」供人确认；**须引用原文**；无引用则跳过并提示「缺少原文依据，已跳过」 | 自动改写检查单已打标签；自动盖「通过/没问题」；无引用仍展示假「需关注」行 |
