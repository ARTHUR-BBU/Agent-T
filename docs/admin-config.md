...
| 单次审查预算（阶段 0.5） | `LLM_BUDGET_PER_REVIEW`（默认 **18**（阶段 3.1 起上调，此前 16/12）；`0`=不限）：预审+评分/补盲+质量层+异议层的 LLM 调用总量，超限走软降级（评分卡 `unavailable(budget_exceeded)`、预审 skip、质量层/异议层 `budget_exceeded`），规则引擎照常。阶段 3.1 全链最坏 16 次（预审 2 + 评分 6 + 质量层 6 + 异议层 2）。**异议层排预算末位**：紧张时最先被牺牲，既有参考层不受稀释 |
...
| 异议层（阶段 3.1） | `OBJECTIONS_ENABLED`（默认 `true`；LLM 对规则结果提疑似误报/漏报候选——**一键回滚开关**，`false` 即关闭且规则引擎零感知，铁律 3：异议永不改档位）；`OBJECTION_TIMEOUT_SECONDS`（未设回落 `LLM_TIMEOUT_SECONDS`）。详见 docs/stage3-class-opinion.md |
