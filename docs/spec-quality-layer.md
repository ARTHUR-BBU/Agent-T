...
- ~~ask 四段式重排（批 2）~~（已落地，见第三节）；~~结果页 IA 重排（批 2）~~（已落地）；~~移动端（批 3）~~（已落地：480px 单断点+五组件+详情独立页，见 docs/design-tokens.md 移动端节）；
- docx 报告塞质量层内容（路线图明令：质量层只在 Web 展示层）；
- 质量层条目参与评分/档位/追问入口。

## 十一、移动端详情/追问屏（阶段 2.4 批 3，2026-09-13 阳仔线框定案）

- **裁定 A**：移动详情页不新建屏——`#screen-ask` 升级兼任（桌面走行内面板→问清楚一点，路径零变化；移动端点条目 `#/review/{rid}/item/{key}` hash 直达）。
- **裁定 B**：阅读顺序=说明→原文→(答案)→追问输入；原文卡不做 sticky（「钉顶」=页头下第一个内容位）。
- **三类条目分流**：仅「需关注且非待核实」出追问卡；待核实（blind）整卡隐藏——不能把「待人工确认」误渲染成「追问暂未开通」。
- **五组件定案**（此前无成文定义）：①立场 segmented 铺满均分 44px ②清单行换行（名称 min-width:0）③评分卡 summary 两行+明细两列 ④详情/追问屏升级 ⑤AI 观察标题行换行。stage/legend/meta/dialog 零改动。
- **路由**：hash=唯一事实源；`history.scrollRestoration="manual"` JS 接管滚动恢复；断点穿越（旋转/拉伸）以当前 hash 重推导视图；零动画。
