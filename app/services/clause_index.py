...
- map_items_to_clauses 是纯展示增强：整段 try/except，任何异常返回空映射，
  绝不让审查失败；只给 item 追加 clause_ids/primary_clause_id 展示字段，
  既有字段一字不动（item.id 是前端→/api/ask→store 三方 join key 红线）。
"""
...
def build_clause_context(
    ...
    装填顺序：**primary 条款居首 → primary 邻居（前/后按距离）→ 其他
    相关条款按文档序（各自带邻居）**——同词（如「违约」）在数十个条款
    出现时，真风险条款（证据坐标所在）可能排在很后，按文档序装填会被
    前面的挤出预算（外部审计二轮 P2）。无 primary 时退化为文档序装填，
    且邻居块内命中条目居首（内部顺序与旧 [i-1,i,i+1] 略有差异，纯增强
    函数、仅影响上下文内部排列）。总量截到 max_chars（延迟护栏）。任何
    异常/未定位返回空串，调用方回退头尾采样——绝不成为追问失败原因。
    """
    try:
